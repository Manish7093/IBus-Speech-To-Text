...
class STTWhisperModelDescription(GObject.Object):
    ...
    def start_downloading(self):
        ...
        #filename = self.name
        #if not filname.endswith(".bin"):
        #    filename +=".bin"
        # Download to cache directory
        #destination = MODEL_DIRS[1] / self.name
...
class STTWhisperLocalModelManager(GObject.Object):
    ...
    def _new_model_available(self, model_path):
        ...
        # Make sure it is a file (Whisper models are .bin files)
        ...
        locale_str = None
        model_type = None
        model_name = model_path.name  # filename without extension

        # Check if it's a standard Whisper model name
        if model_name.startswith("ggml-"):
            # Extract model type (tiny, base, small, medium, large)
            # ggml-tiny.bin, ggml-base.en.bin, ggml-large-v3.bin
            parts = model_name.replace("ggml-", "").split(".")
            model_type = parts[0]

            # Check for English-only models
            #if len(parts) > 1 and parts[1] == "en":
            if ".en" in model_name:
                locale_str = "en"
                model_type += ".en"
            else:
                # Multilingual models - assign to common locales
    ...
    def get_models_for_locale(self, locale_str):
        # For Whisper, also return multilingual models
...
class STTWhisperOnlineModelManager(GObject.Object):
    ...
    def _populate_with_whisper_models(self):
        """Populate with standard Whisper models"""
        ...
        for model_name, filename in WHISPER_MODELS.items():
            ...
            # Avoid duplicate entries - check if already exists
        ...
        # Add locally available custom models
        for locale in stt_whisper_local_model_manager().get_supported_locales():
            ...
            for model_desc in model_list:
                ...
                # For standard models, check if already exists with .bin extension
    ...
    def get_models_for_locale(self, locale_str):
        # Return models for locale + multilingual models
        models = self._locales_dict.get(locale_str, []).copy()
